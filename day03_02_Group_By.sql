...
SELECT DISTINCT urun_adi, SUM(miktar_kg)
FROM market
WHERE urun_adi = 'muz' ;
...
SELECT kategori, SUM(miktar_kg * satis_fiyati) AS 'kategori toplam hasilat'
FROM market
GROUP BY kategori;
...
SELECT DISTINCT urun_adi, SUM(miktar_kg) AS toplam_satis_miktari
FROM market
GROUP BY urun_adi
...
SELECT satis_tarihi, AVG(satis_fiyati)
FROM market
...
SELECT urun_adi, AVG(satis_fiyati) AS 'urun ortalama fiyati'
FROM market
GROUP BY urun_adi;
